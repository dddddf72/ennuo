...
router.get("/getbanner", async (req, res) => {
	let data = await Db.select(
		req,
		`SELECT * FROM ${tableNameBanner} WHERE status = 1`
	);
...
router.get("/getseckill", async (req, res) => {
	...
	var sql = `SELECT a.*,b.img,b.price FROM ${tableNameSeck} a LEFT JOIN ${tableNameGoods} b ON a.goodsid = b.id WHERE begintime >= ${start} AND endtime <= ${end}`;
	console.log(sql);
...
router.get("/getindexgoods", async (req, res) => {
	let data1 = await Db.select(
		req,
		`SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND ishot = 1 LIMIT 10`
	);
	let data2 = await Db.select(
		req,
		`SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND isnew = 1 LIMIT 10`
	);
	let data3 = await Db.select(
		req,
		`SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 LIMIT 10`
	);
	let data = [];
	...
	if (fid || sid) {
		...
		if (fid) {
			data = await Db.select(
				req,
				`SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND first_cateid = ${fid}`
			);
			res.send(Success(data));
			return;
		}
		if (sid) {
			data = await Db.select(
				req,
				`SELECT id,goodsname,price,market_price,img FROM ${tableNameGoods} WHERE status = 1 AND second_cateid = ${sid}`
			);
			res.send(Success(data));
			return;
		}
...
router.get("/getgoodsinfo", async (req, res) => {
	...
	const info = await Db.select(
		req,
		`SELECT a.*,b.specsname FROM ${tableNameGoods} a LEFT JOIN ${tableNameSpecs} b ON a.specsid = b.id WHERE a.id = '${id}'`
	);
...
router.post("/register", async (req, res) => {
	...
	const info = await Db.select(
		req,
		`SELECT * FROM ${tableNameMember} WHERE phone = '${phone}'`
	);
	if (info) {
